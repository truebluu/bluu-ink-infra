...
def local_review(gd_path: Path, dept: str, task: dict) -> tuple[bool, list[str]]:
    ...
    if not (has_class or has_extend):
    ...
    elif not has_func and not (has_signal or has_export):
        ...
        # Also reject bodies that are ONLY a var/const declaration or a
        # trivial literal return (kimi + gpt-oss flagged these as false
        # negatives — `func foo(): var x = 0` does no work).
        real_work = False
